...
| `verbose-logging` | Show infrastructure identifiers in the action log: account ID, S3 bucket name, application and environment names, version label, environment URL and ID, container image URIs, and the Elastic Beanstalk event stream (which names resources the service creates, such as security groups, load balancers, and instances). Set to `false` to mask those identifiers as `***`, suppress event details, and strip AWS resource IDs (instance, security group, subnet, VPC, ENI IDs, ARNs, IP addresses, environment IDs, ECR image references) from failure messages. Action outputs are always set either way and remain readable by later steps in the same job. Note that GitHub does not forward job outputs whose value matches a masked secret, so if you promote `environment-url`, `environment-id`, or `version-label` to a job-level output for other jobs, leave this `true` | `true` |
